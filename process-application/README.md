# BusinessProcessAutomation Lab

## Components

The BPA Lab project contains following elements :

src/main: Process Application for [Camunda Platform](http://docs.camunda.org) representing a manufacturing and logistics process including user tasks, external workes for IOT Device and external workers for Fischertechnik Robot

src/CamundaIoTJavaScript: Java scripts for IoT Device: CamundaWorker contains external workers for Camunda; IoTGateway contains scripts running on IOT Gateway (Rasberry PI) based on Node.JS

src/WarehouseRobotPythonScripts: python scripts including external worker for Camunda and scipts for Fischertechnik warehouse robot control based on ftrobopy library

src/ProcessReporting: configuration file for Microsoft PowerBI displaying data in MongoDB collections

# D R A F T - to be defined
...
## BPMN Models
[BPMN Process](src/main/resources/process.bpmn) with subprocess (src/main/resources/warehouseOperation_run.bpmn). Both are deployed with .war file on Camunda BPM platform 
...
### Unit Test
You can run the JUnit test [ProcessTest](src/test/java/org/praxisproject/Camunda_IoT/ProcessTest.java) in your IDE or using:
...
<!-- Tweet
New @Camunda example: Camunda Platform Process Application - A Process Application for [Camunda Platform](http://docs.camunda.org). https://github.com/camunda-consulting/code/tree/master/snippets/Camunda-IoT
-->
